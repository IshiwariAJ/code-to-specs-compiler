...
[![Tests](https://img.shields.io/badge/tests-530%20passing-brightgreen.svg)](#テスト--testing)
...
| ⚡ **高速・決定論的** | 同じ入力からは常に同じ出力（530テスト全グリーン） |
...
| テストファイル | 件数 | 内容 |
|---|---|---|
| `tests/test_mapper.py` | 274件 | AST → IR マッピングのユニットテスト |
| `tests/test_renderer.py` | 125件 | IR → Markdown レンダリングのユニットテスト |
| `tests/test_pipeline.py` | 92件 | E2E 統合テスト（TypeScript / Python / Go / PowerShell / Java） |
...
| ⚡ **Fast & deterministic** | Same input always produces the same output (530 tests passing) |
...
| File | Count | Scope |
|---|---|---|
| `tests/test_mapper.py` | 274 | AST → IR mapping unit tests |
| `tests/test_renderer.py` | 125 | IR → Markdown rendering unit tests |
| `tests/test_pipeline.py` | 92 | E2E integration tests (TypeScript / Python / Go / PowerShell / Java) |
